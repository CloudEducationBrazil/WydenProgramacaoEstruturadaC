...
int totaluser = 1;
char nome[numeroUSER][100] = {"ADMINISTRADOR", "SÉRGIO NOVAIS"}; //inserção de 1 usuário administrador e 1 usuário cliente pré-definidos
int conta[numeroUSER] = {1,2}; //Número da conta
int agencia[numeroUSER] = {1,2}; // Número da agência
float limite[numeroUSER] = {10000,457};// Limite do cartão
...
void cadastrarCliente(){
	...
	scanf("%d", &n_agencia);
	printf("\tDigite um número de limite: ");
	scanf("%f", &n_limite);
	printf("\tDigite um número de senha: ");
	scanf("%d", &n_senha);
	printf("\tDigite um número de tipo (0 - CLIENTE, 1 - ADMINISTRADOR): ");
	scanf("%d", &n_tipo);
